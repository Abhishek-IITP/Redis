...
const PRODUCT_CACHE_KEY ="product:all"
...
export async function getAllProducts(filters: {
  ...
  if(hasFilters){
    console.log("cache bypass: filtere product list");
  ...
  if(cachedProducts){
    ...
    const items = await FetchAllProductsFromDatabase(filters)
...
export async function getProductById(id: number): Promise<Product | null> {
  const cachedKey = getProductCacheKey(id);

  const cachedProducts = await redis.get(cachedKey);
  if(cachedProducts){
  ...
  await redis.setEx(cachedKey,PRODUCT_CACHE_TTL, JSON.stringify(product));

  console.log("cache set", cachedKey)

  return product;
  

}
async function deleteAllCachedProduct():Promise<void>{
  await redis.del(PRODUCT_CACHE_KEY);
  console.log("cache deleted: products all")
}
async function deleteSingleCachedProduct(productId:number):Promise<void>{
  const cachedKey = getProductCacheKey(productId);
  await redis.del(cachedKey);
  console.log("cache deleted: single Products")
}
...
export async function updateProduct(
  ...
  await deleteAllCachedProduct();
  await deleteSingleCachedProduct(id);
